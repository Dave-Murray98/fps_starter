...
public class SaveManager : MonoBehaviour
{
    ...
    private bool LoadSaveDataFromFile()
    {
        ...
        try
        {
            ...
            // ADD THIS LINE to debug the loaded structure
            DebugLoadedSaveStructure();
    ...
    /// <summary>
    /// Debug the save data structure after loading from file
    /// </summary>
    [Button]
    [System.Diagnostics.Conditional("UNITY_EDITOR")]
    private void DebugLoadedSaveStructure()
    {
        if (currentSaveData == null)
        {
            DebugLog("No save data loaded");
            return;
        }

        DebugLog("=== LOADED SAVE DATA STRUCTURE ===");
        DebugLog($"Save Time: {currentSaveData.saveTime}");
        DebugLog($"Current Scene: {currentSaveData.currentScene}");

        // Debug PlayerSaveData
        if (currentSaveData.playersaveData != null)
        {
            DebugLog($"\nPlayerSaveData:");
            DebugLog($"  Health: {currentSaveData.playersaveData.currentHealth}");
            DebugLog($"  Position: {currentSaveData.playersaveData.position}");
            DebugLog($"  Custom Data Count: {currentSaveData.playersaveData.CustomDataCount}");

            DebugLog($"  Custom Data Entries:");
            foreach (string key in currentSaveData.playersaveData.GetCustomDataKeys())
            {
                ...
                if (data is InventorySaveData invData)
                {
                    DebugLog($"      Inventory: {invData.ItemCount} items in {invData.gridWidth}x{invData.gridHeight} grid");
                    DebugLog($"      Next Item ID: {invData.nextItemId}");
                }
                else if (data is EquipmentSaveData eqData)
                {
                    var assignedCount = eqData.hotkeyBindings?.FindAll(h => h.isAssigned)?.Count ?? 0;
                    DebugLog($"      Equipment: {assignedCount} hotkey assignments");
                    DebugLog($"      Equipped Item: {eqData.equippedItem?.isEquipped == true}");
                }
            }
        }
        else
        {
            DebugLog("PlayerSaveData: null");
        }

        // Debug PlayerPersistentData
        if (currentSaveData.playerPersistentData != null)
        {
            DebugLog($"\nPlayerPersistentData:");
            DebugLog($"  Health: {currentSaveData.playerPersistentData.currentHealth}");
            DebugLog($"  Component Data Count: {currentSaveData.playerPersistentData.ComponentDataCount}");

            DebugLog($"  Component Entries:");
            foreach (string componentId in currentSaveData.playerPersistentData.GetStoredComponentIDs())
            {
                var compData = currentSaveData.playerPersistentData.GetComponentData<object>(componentId);
                DebugLog($"    {componentId}: {compData?.GetType().Name ?? "null"}");
            }
        }
        else
        {
            DebugLog("PlayerPersistentData: null");
        }

        // Debug Scene Data
        if (currentSaveData.sceneData != null)
        {
            DebugLog($"\nScene Data: {currentSaveData.sceneData.Count} scenes");
            foreach (var kvp in currentSaveData.sceneData)
            {
                DebugLog($"  {kvp.Key}: {kvp.Value.objectData.Count} objects");
            }
        }
        else
        {
            DebugLog("Scene Data: null");
        }
    }
    ...
    /// <summary>
    /// Debug the transition data being prepared
    /// </summary>
    [Button]
    [System.Diagnostics.Conditional("UNITY_EDITOR")]
    public void DebugTransitionDataPreparation()
    {
        if (currentSaveData == null)
        {
            DebugLog("No current save data to debug");
            return;
        }

        DebugLog("=== DEBUGGING TRANSITION DATA PREPARATION ===");

        var transitionData = PrepareSaveDataForTransition();

        DebugLog($"Prepared transition data with {transitionData.Count} entries:");
        ...
        foreach (var kvp in transitionData)
        {
            ...
            if (kvp.Value is PlayerPersistentData ppd)
            {
                DebugLog($"  PlayerPersistentData Details:");
                DebugLog($"    Health: {ppd.currentHealth}");
                DebugLog($"    Component Count: {ppd.ComponentDataCount}");
                ...
                foreach (string componentId in ppd.GetStoredComponentIDs())
                {
                    ...
                    {
                        DebugLog($"      Inventory Details: {invData.ItemCount} items, nextID: {invData.nextItemId}");
                    }
                    else if (compData is EquipmentSaveData eqData)
                    {
                        var assignedCount = eqData.hotkeyBindings?.FindAll(h => h.isAssigned)?.Count ?? 0;
                        DebugLog($"      Equipment Details: {assignedCount} hotkeys, equipped: {eqData.equippedItem?.isEquipped}");
                    }
                }
            }
            else if (kvp.Value is PlayerSaveData psd)
            {
                DebugLog($"  PlayerSaveData Details:");
                DebugLog($"    Health: {psd.currentHealth}");
                DebugLog($"    Position: {psd.position}");
                DebugLog($"    Custom Data Count: {psd.CustomDataCount}");
            }
        }
    }
